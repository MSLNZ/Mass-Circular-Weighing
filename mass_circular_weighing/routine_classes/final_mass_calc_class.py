...
def num_to_eng_format(num) -> str:
    """Format a mass value as a string in engineering format, where the smallest unit is µg"""
    for key, val in SUFFIX.items():
        renum = num/val
        if abs(renum) < 1000:
            if key == 'ng':
                return "{} {}".format(round(renum/1000, 3), MU_STR+'g')
